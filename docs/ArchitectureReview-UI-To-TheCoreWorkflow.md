...
**Date:** 2026-09-18
**Scope:** `CoreChatPage` → `CoreChatViewModel.SendAsync` → `IOrchestrationControl.InitializeOrchestrationAsync` → `OrchestrationFactory` → `TheCoreWorkflow.InitializeAsync` / `ExecuteAsync` → executor graph → `WorkflowExecutionResult` → UI
**Supporting cast reviewed:** DI composition (`SentinelCoreServiceExtensions`, `ExecutorRegistration`, `App.xaml.cs`), `SentinelAgentFactory`, `WorkflowBase`, `SentinelWorkflowExecution`, `ISentinelCoreEvents` event hub, terminal executors.
...
**Bottom line:** The single most urgent item is **C1** — as written, the chat works exactly once per application run.
