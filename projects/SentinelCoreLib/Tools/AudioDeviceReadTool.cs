...
public sealed class AudioDeviceReadTool : AITool
{
    ...
    public Task<ToolResult> audio_list_devices()
    {
        try
        {
            ...
            {
                return Task.FromResult(ToolResult.FailureResult("Unable to create MMDeviceEnumerator."));
            }
        ...
        {
            return Task.FromResult(ToolResult.FailureResult($"Audio device listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> audio_read_default_device()
    {
        try
        {
            ...
            if (hr < 0)
            {
                return Task.FromResult(ToolResult.FailureResult("No default render endpoint found."));
            }

            using MarshalReleaseScope deviceCom = new(device);
            device.GetId(out string id);
            device.GetState(out int state);
            return Task.FromResult(ToolResult.SuccessResult($"DefaultRenderEndpoint Id={id} State={state}"));
        }
        catch (Exception ex)
        {
            return Task.FromResult(ToolResult.FailureResult($"Default audio device read failed: {ex.Message}"));
        }
    }
