...
public sealed class HyperVReadTool : AITool
{
    ...
    public Task<ToolResult> hyperv_list_switches()
    {
        try
        {
            ...
            {
                string name = switchObj.CimInstanceProperties["Name"]?.Value?.ToString() ?? string.Empty;
                string elementName = switchObj.CimInstanceProperties["ElementName"]?.Value?.ToString() ?? string.Empty;
        ...
        {
            return Task.FromResult(ToolResult.FailureResult($"Hyper-V switch listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> hyperv_list_vms()
    {
        ...
        {
            return Task.FromResult(ToolResult.FailureResult($"Hyper-V VM listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> hyperv_read_vm([Description("The VM name (ElementName).")] string vmName)
    {
        try
        {
            ...
            {
                return Task.FromResult(ToolResult.FailureResult("vmName is required."));
            }
            ...
            foreach (CimInstance? vm in session.QueryInstances(HyperVNamespace, "WQL", query))
            {
                ...
                foreach (CimProperty? property in vm.CimInstanceProperties)
                    ...
                    {
                            Array array => string.Join("|", array.Cast<object>().Select(x => x?.ToString() ?? string.Empty)),
                            _ => property.Value
                    };
            ...
            if (results.Count == 0)
            {
                return Task.FromResult(ToolResult.FailureResult($"Hyper-V VM not found: {vmName}"));
            }
        ...
        {
            return Task.FromResult(ToolResult.FailureResult($"Hyper-V VM read failed: {ex.Message}"));
        }
    }
}