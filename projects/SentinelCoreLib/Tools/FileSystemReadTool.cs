...
public sealed class FileSystemReadTool : AITool
{
    ...
    public Task<ToolResult> file_system_list_directory([Description("The absolute directory path to list.")] string path, [Description("Optional search pattern, e.g. *.txt. Defaults to *.")] string? searchPattern = null)
    {
        try
        {
            ...
            {
                return Task.FromResult(ToolResult.FailureResult("path is required."));
            }

            DirectoryInfo dir = new(path);
            if (!dir.Exists)
            {
                return Task.FromResult(ToolResult.FailureResult($"Directory not found: {path}"));
            }
        ...
        {
            return Task.FromResult(ToolResult.FailureResult($"Directory listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> file_system_read_acl([Description("The absolute file or directory path to inspect.")] string path)
    {
        try
        {
            ...
            {
                return Task.FromResult(ToolResult.FailureResult($"Path not found: {path}"));
            }

            StringBuilder sb = new();
            sb.AppendLine($"Path={path}");
            sb.AppendLine($"Owner={security.GetOwner(typeof(System.Security.Principal.NTAccount))}");
            sb.AppendLine($"Group={security.GetGroup(typeof(System.Security.Principal.NTAccount))}");
            sb.AppendLine("AccessRules:");
            foreach (FileSystemAccessRule rule in security.GetAccessRules(true, true, typeof(System.Security.Principal.NTAccount))) sb.AppendLine($"  Identity={rule.IdentityReference}, Rights={rule.FileSystemRights}, Type={rule.AccessControlType}, Inheritance={rule.InheritanceFlags}, Propagation={rule.PropagationFlags}");

            return Task.FromResult(ToolResult.SuccessResult(sb.ToString()));
        }
        catch (Exception ex)
        {
            return Task.FromResult(ToolResult.FailureResult($"ACL read failed: {ex.Message}"));
        }
    }
        ...
        try
        {
            ...
            if (!info.Exists)
            {
                ...
                if (dirInfo.Exists)
                {
                    ...
                    return Task.FromResult(ToolResult.SuccessResult(JsonSerializer.Serialize(dirResult, new JsonSerializerOptions { WriteIndented = true })));
                }
            ...
            return Task.FromResult(ToolResult.SuccessResult(JsonSerializer.Serialize(fileResult, new JsonSerializerOptions { WriteIndented = true })));
        }
        catch (Exception ex)
        {
            return Task.FromResult(ToolResult.FailureResult($"File system metadata read failed: {ex.Message}"));
        }
    }
}