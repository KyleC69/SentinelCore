...
public sealed class LocalAccountsReadTool : AITool
{
    ...
    public Task<ToolResult> local_group_list([Description("Optional group name to filter. If provided, members of that group are listed.")] string? groupName = null)
    {
        try
        {
            ...
            foreach (GroupPrincipal group in searcher.FindAll().Cast<GroupPrincipal>())
            {
                ...
                foreach (Principal member in group.GetMembers()) sb.AppendLine($"  Member={member.Name} ({member.StructuralObjectClass})");
            }
        ...
        {
            return Task.FromResult(ToolResult.FailureResult($"Local group listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> local_user_list()
    {
        try
        {
            ...
            foreach (UserPrincipal user in searcher.FindAll().Cast<UserPrincipal>()) sb.AppendLine($"Name={user.Name}, Enabled={user.Enabled}, LastLogon={user.LastLogon}, PasswordNeverExpires={user.PasswordNeverExpires}, UserCannotChangePassword={user.UserCannotChangePassword}");

            return Task.FromResult(ToolResult.SuccessResult(sb.ToString()));
        }
        catch (Exception ex)
        {
            return Task.FromResult(ToolResult.FailureResult($"Local user listing failed: {ex.Message}"));
        }
    }
}