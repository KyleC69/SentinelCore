...
public sealed partial class McpServersViewModel : ObservableObject, INavigationAware, IDisposable
{
    ...
    [ObservableProperty]
    private string _newServerArgumentsText = string.Empty;

    [ObservableProperty]
    private McpServerTransportType _newServerTransportType = McpServerTransportType.Stdio;

    [ObservableProperty]
    private string _newServerWorkingDirectory = string.Empty;

    [ObservableProperty]
    private string _newServerEnvironmentText = string.Empty;
    ...
    /// </summary>
    public bool IsStdioTransport => NewServerTransportType == McpServerTransportType.Stdio;
    ...
    private async Task LoadSelectedServerAssignmentsAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            foreach (string agent in agents)
            {
                ...
                {
                    AgentName = agent,
                    IsAssigned = info.Definition.AssignedAgentNames.Count == 0
                        || info.Definition.AssignedAgentNames.Contains(agent, StringComparer.OrdinalIgnoreCase)
                });
            }
    ...
    /// </summary>
    /// <remarks>Use to release resources or persist transient state before navigation. Avoid long-running
    ...
    private bool CanAddServer() =>
        !IsBusy
        && !string.IsNullOrWhiteSpace(NewServerDisplayName)
        && !string.IsNullOrWhiteSpace(NewServerCommandOrEndpoint)
        && IsAddingServer;
    ...
    private bool CanStartServer() =>
        ...
        && SelectedServer.Status is not McpServerStatus.Starting
        && SelectedServer.Status is not McpServerStatus.Connected;
    ...
    private bool CanStopServer() =>
        !IsBusy
        && SelectedServer is not null
        && SelectedServer.Status is not McpServerStatus.Stopped
        && SelectedServer.Status is not McpServerStatus.Starting;
    ...
    private async Task AddServerAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            IReadOnlyList<string> assignedAgents = NewServerAgentAssignments
                .Where(r => r.IsAssigned)
                .Select(r => r.AgentName)
                .ToList();

            McpServerDefinition definition = new(
                id,
                NewServerDisplayName.Trim(),
                NewServerTransportType,
                NewServerCommandOrEndpoint.Trim(),
                arguments,
                string.IsNullOrWhiteSpace(NewServerWorkingDirectory) ? null : NewServerWorkingDirectory.Trim(),
                environment.Count > 0 ? environment : null,
                assignedAgentNames: assignedAgents.Count > 0 ? assignedAgents : null);

            await _registry.RegisterAsync(definition, token).ConfigureAwait(false);
    ...
    private async Task UpdateAssignmentsAsync(CancellationToken token)
    {
        ...
        try
        {
            List<string> assigned = SelectedServerAgentAssignments
                .Where(r => r.IsAssigned)
                .Select(r => r.AgentName)
                .ToList();
    ...
    private void ApplyLoadedData(List<McpServerRow> rows, List<string> agentNames)
    {
        ...
        SelectedServer = selectedId is null
            ? null
            : Servers.FirstOrDefault(s => s.Id == selectedId);
    }
    ...
    private static McpServerRow MapToRow(McpServerInfo server)
    {
        return new McpServerRow
        {
            ...
            AssignedAgentsText = server.Definition.AssignedAgentNames.Count > 0
                ? string.Join(", ", server.Definition.AssignedAgentNames)
                : "All agents",
