...
public sealed class PersistenceRegistrationTests
{
    ...
    private const string TestConnectionString =
        "Server=(localdb)\\mssqllocaldb;Database=SentinelCore.PatternLockTests;Trusted_Connection=True";
    ...
    public void Factory_Consumers_Fail_Activation_When_The_Factory_Registration_Is_Missing()
    {
        ...
        InvalidOperationException exception = Assert.Throws<InvalidOperationException>(
            () => provider.GetRequiredService<ICaseFlowEngine>());
    ...
    public void Locked_Persistence_Consumers_Inject_Only_The_DbContext_Factory()
    {
        ...
        foreach (Type consumer in lockedConsumers)
        {
            ...
            Assert.IsTrue(
                ...
                $"{consumer.Name} must expose exactly one public constructor.");

            ParameterInfo[] parameters = constructors[0].GetParameters();
            Assert.IsTrue(
                parameters.Length == 1,
                $"{consumer.Name} must inject only the DbContext factory — direct context injection is prohibited.");

            Assert.AreEqual(
                factoryType,
                parameters[0].ParameterType,
                $"{consumer.Name} must inject IDbContextFactory<SentinelCoreDBContext> (PL-7 rule 2).");
        }
    }
    ...
    public async Task CaseFlowEngine_Creates_Its_Context_From_The_Factory_Per_OperationAsync()
    {
        ...
        CaseFlowEngine engine = new(factory.Object);

        // Act & Assert — the engine must reach the context through the factory.
        InvalidOperationException actual = await Assert.ThrowsAsync<InvalidOperationException>(
            () => engine.GetCaseCountByStatusAsync(CaseStatus.Open));
    ...
    public async Task EvidenceStore_Creates_Its_Context_From_The_Factory_Per_OperationAsync()
    {
        ...
        // Act & Assert
        InvalidOperationException actual = await Assert.ThrowsAsync<InvalidOperationException>(
            () => store.GetByCaseIdAsync(Guid.NewGuid().ToString()));
    ...
    public async Task SignalRepository_Creates_Its_Context_From_The_Factory_Per_OperationAsync()
    {
        ...
        // Act & Assert
        InvalidOperationException actual = await Assert.ThrowsAsync<InvalidOperationException>(
            () => repository.AddAsync(new Signal("pattern-lock conformance", "unit-test")));
