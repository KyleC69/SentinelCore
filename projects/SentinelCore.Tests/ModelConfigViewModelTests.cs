...
public sealed class ModelConfigViewModelTests
{
    ...
    private static (Mock<ISentinelAgentCatalog> Catalog, Mock<IModelConfigStore> Store, SentinelCoreSettings Settings, IOptions<SentinelCoreSettings> Options) CreateDependencies()
    {
        ...
        catalog.Setup(c => c.GetAgentNamesAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(["TheCore", "Manager", "Worker1"]);
    ...
    {
        // Arrange
        var (_, store, _, options) = CreateDependencies();

        // Act & Assert
        Assert.Throws<ArgumentNullException>(
            () => new ModelConfigViewModel(null!, store.Object, options, NullLogger<ModelConfigViewModel>.Instance));
    }
    ...
    {
        // Arrange
        var (catalog, _, _, options) = CreateDependencies();

        // Act & Assert
        Assert.Throws<ArgumentNullException>(
            () => new ModelConfigViewModel(catalog.Object, null!, options, NullLogger<ModelConfigViewModel>.Instance));
    }
    ...
    {
        // Arrange
        var (catalog, store, _, _) = CreateDependencies();

        // Act & Assert
        Assert.Throws<ArgumentNullException>(
            () => new ModelConfigViewModel(catalog.Object, store.Object, null!, NullLogger<ModelConfigViewModel>.Instance));
    }
    ...
    {
        // Arrange
        var (catalog, store, _, options) = CreateDependencies();

        // Act & Assert
        Assert.Throws<ArgumentNullException>(
            () => new ModelConfigViewModel(catalog.Object, store.Object, options, null!));
    }
    ...
    {
        // Arrange
        var (catalog, store, settings, options) = CreateDependencies();
    ...
    public async Task SaveCommand_CompleteCards_PersistsAndAppliesAsync()
    {
        ...
        store.Verify(s => s.Save(It.Is<ModelConfigDocument>(d =>
            d.AgentModels.Count == 3
            && d.AgentModels.ContainsKey("TheCore")
            && d.AgentModels.ContainsKey("Manager")
            && d.AgentModels.ContainsKey("Worker1"))), Times.Once);
    ...
    public async Task SaveCommand_StoreThrows_ReportsErrorAsync()
    {
        // Arrange
        var (catalog, store, _, options) = CreateDependencies();
        store.Setup(s => s.Save(It.IsAny<ModelConfigDocument>()))
