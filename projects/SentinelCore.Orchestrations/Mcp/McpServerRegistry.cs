...
public sealed class McpServerRegistry : IMcpServerRegistry
{
    ...
    public async Task StartAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ModelContextProtocol.Client.McpClient client = await _connectionFactory
                .ConnectAsync(entry.Definition, cancellationToken)
                .ConfigureAwait(false);

            IList<ModelContextProtocol.Client.McpClientTool> tools = await client
            ...
            _logger.LogInformation(
                "MCP server {ServerId} connected with {ToolCount} tool(s).",
                id,
                entry.ToolNames.Count);
        }
        catch (Exception ex)
        {
    ...
    public async Task StopAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        ModelContextProtocol.Client.McpClient? client = entry.Client;
        entry.Client = null;
    ...
    public async Task UpdateAssignmentsAsync(string id, IReadOnlyList<string> assignedAgentNames, CancellationToken cancellationToken = default)
    {
        ...
        McpServerDefinition updated = new(
            current.Id,
            current.DisplayName,
            current.TransportType,
            current.CommandOrEndpoint,
            current.Arguments,
            current.WorkingDirectory,
            current.EnvironmentVariables,
            current.OAuthSettings,
            assignedAgentNames);
        _entries[id] = new McpServerEntry(updated)
        {
            Status = entry.Status,
    ...
    {
        IReadOnlyList<McpServerInfo> infos = _entries.Values
            .Select(e => e.ToInfo())
            .ToList()
            .AsReadOnly();
    ...
    public async Task<IReadOnlyList<AITool>> GetToolsForAgentAsync(string agentName, CancellationToken cancellationToken = default)
    {
        ...
        foreach (McpServerEntry entry in _entries.Values)
        {
            ...
            try
            {
                IList<McpClientTool> serverTools = await entry.Client
                    .ListToolsAsync(cancellationToken: cancellationToken)
                    .ConfigureAwait(false);
    ...
    {
        IReadOnlyList<McpServerDefinition> definitions = _entries.Values
    ...
    private sealed class McpServerEntry
    {
        ...
        public ModelContextProtocol.Client.McpClient? Client
        {
            get => _client;
            set => _client = value;
        }
