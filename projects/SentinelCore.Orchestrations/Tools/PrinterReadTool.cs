...
public sealed class PrinterReadTool : AITool
{
    ...
    public override string Description { get; } = "Read-only tool for querying printer configuration and queues via the Print Spooler API.";
    public override string Name { get; } = "Printer_Read";
    ...
    public Task<ToolResult> printer_list()
    {
        try
        {
            ...
            if (!EnumPrinters(Flags, null, LevelTwo, IntPtr.Zero, 0, out uint needed, out uint returned) && needed == 0)
                return Task.FromResult(ToolResult.Fail($"Printer enumeration failed: {PInvokeHelper.GetLastErrorMessage()}"));
        ...
        {
            return Task.FromResult(ToolResult.Fail($"Printer listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> printer_read([Description("The printer name to inspect.")] string printerName)
    {
        try
        {
            ...
            {
                return Task.FromResult(ToolResult.Fail("printerName is required."));
            }

            if (!OpenPrinter(printerName, out IntPtr hPrinter, IntPtr.Zero))
            {
                return Task.FromResult(ToolResult.Fail($"OpenPrinter failed: {PInvokeHelper.GetLastErrorMessage()}"));
            }
            ...
            try
            {
                if (!GetPrinter(hPrinter, 2, IntPtr.Zero, 0, out int needed))
                {
                    if (needed == 0)
                    {
                        return Task.FromResult(ToolResult.Fail($"GetPrinter failed: {PInvokeHelper.GetLastErrorMessage()}"));
                    }
                }
        ...
        {
            return Task.FromResult(ToolResult.Fail($"Printer read failed: {ex.Message}"));
        }
    }
