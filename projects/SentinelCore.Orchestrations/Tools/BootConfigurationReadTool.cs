...
public sealed class BootConfigurationReadTool : AITool
{
    ...
    public override string Description { get; } = "Read-only tool for querying system boot configuration using BCD store APIs.";
    public override string Name { get; } = "Boot_Configuration_Read";
    ...
    public Task<ToolResult> bcdedit_current()
    {
        try
        {
            ...
            if (hResult < 0 || store == IntPtr.Zero)
                return Task.FromResult(ToolResult.Fail("Failed to open the BCD store."));
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (hResult < 0)
                    {
                        return Task.FromResult(ToolResult.Fail("Failed to query current BCD entry."));
                    }

                    Guid currentGuid = Marshal.PtrToStructure<Guid>(guid);
                    return Task.FromResult(ToolResult.Ok($"CurrentBootEntry={currentGuid:B}"));
                }
                finally
                {
        ...
        {
            return Task.FromResult(ToolResult.Fail($"BCD current entry read failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> bcdedit_enum()
    {
        try
        {
            ...
            try
            {
                ...
                if (hResult < 0)
                {
                    return Task.FromResult(ToolResult.Fail("Failed to enumerate BCD entries."));
                }
        ...
        {
            return Task.FromResult(ToolResult.Fail($"BCD enumeration failed: {ex.Message}"));
        }
    }
