...
public sealed class InstalledAppsReadTool : AITool
{
    ...
    public override string Description { get; } = "Read-only tool for enumerating installed applications via MSI registry entries and Win32_Product CIM class.";
    public override string Name { get; } = "Installed_Apps_Read";
    ...
    public Task<ToolResult> installed_apps_list([Description("Optional publisher or display name filter (partial match).")] string? filter = null)
    {
        ...
        {
            return Task.FromResult(ToolResult.Fail($"Installed app listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> installed_apps_msi_list([Description("Optional product name filter (partial match).")] string? filter = null)
    {
        ...
        {
            return Task.FromResult(ToolResult.Fail($"MSI product listing failed: {ex.Message}"));
        }
    }
}