...
{
    public override string Description { get; } = "Read-only tool for querying Windows Event Logs.";
    public override string Name { get; } = "Event_Log_Read";
    ...
    public Task<ToolResult> event_log_list_channels()
    {
        ...
        {
            return Task.FromResult(ToolResult.Fail($"Event log channel listing failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> event_log_query([Description("The event log channel name, e.g. Application or System.")] string channel, [Description("Optional XPath filter expression. Defaults to all events.")] string? query = null, [Description("Maximum number of events to return. Defaults to 50.")] int maxEvents = 50)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(channel))
                return Task.FromResult(ToolResult.Fail("channel is required."));
        ...
        {
            return Task.FromResult(ToolResult.Fail($"Event log query failed: {ex.Message}"));
        }
    }
    ...
    public Task<ToolResult> event_log_read_configuration([Description("The event log channel name.")] string channel)
    {
        try
        {
            ...
            return Task.FromResult(ToolResult.Ok(JsonSerializer.Serialize(result, new JsonSerializerOptions { WriteIndented = true })));
        }
        catch (Exception ex)
        {
            return Task.FromResult(ToolResult.Fail($"Event log configuration read failed: {ex.Message}"));
        }
    }
}